...
*   右侧面板显示了所有的提交历史。
*   **Restore Copy (推荐)**: 选中一个文件后，点击此按钮。系统会将该版本的文档提取出来，作为一个新文件保存在项目根目录下的 `Restore Copy` 文件夹中。这非常安全，不会覆盖您当前的工作。
*   **Revert (Dev)**: 尝试创建一个撤销提交。由于 Word 文档的特殊性，这可能会导致冲突，建议仅供开发者调试使用。
